...
ggplot(data) + geom_line(aes(Year, Diameter))
```

## colourを指定

```{r geom_line4, fig.width=5, fig.height=2.75, echo=TRUE}
ggplot(data) + geom_line(aes(Year, Diameter, colour = No)) +
...
ggplot(data) + geom_line(aes(Year, Diameter, group = No))
```
...
p <- ggplot(iris) +
  geom_density(aes(Sepal.Length, fill = Species), alpha = 0.5)
print(p)
```
...
p1 <- ggplot(data1) + geom_line(aes(Year, Level))
print(p1)
```
...
p1 + geom_line(data = data2, aes(Year, Filtered), colour = "red") +
  geom_ribbon(data = data2,
              aes(Year, ymin = Lower, ymax = Upper), alpha = 0.5)
```

## 空間点分布

```{r point_data, fig.width=5, fig.height=2.75, include=FALSE}
library(readr)
point_data <- read_csv("point_data.csv")
```

```{r point_data2, fig.width=5, fig.height=2.75, echo=TRUE}
p <- ggplot(point_data, aes(x, y))
p + geom_point()
```
...
```{r density_2d, fig.width=5, fig.height=2.75, echo=TRUE}
p + geom_density_2d()
```

## 密度のポリゴングラフ

```{r density_2d_polygon, fig.width=5, fig.height=2.75, echo=TRUE}
