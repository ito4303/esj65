...
```{r colour3, fig.width=5,fig.height=3,echo=TRUE}
...
```{r coord, fig.width=5,fig.height=2.1,echo=TRUE}
p2 <- ggplot(g_data) + xlim(0, 50) +
  scale_y_reverse(limits = c(50, 0)) + coord_equal()
p2 + geom_point(mapping = aes(x = X, y = Y, colour = Species))
```

## 種類ごとに点の形をかえる

```{r point_shape,fig.width=5,fig.height=2.1,echo=TRUE}
...
```{r point_size,fig.width=5,fig.height=2.1,echo=TRUE}
p2 + geom_point(aes(x = X, y = Y,
                    colour = Species, shape = Species),
                size = 4, alpha = 0.7)
```
...
```{r point_size2,fig.width=5,fig.height=2.1,echo=TRUE}
p3 <- p2 + geom_point(aes(x = X, y = Y, 
...
```{r spec_colour,fig.width=5,fig.height=2.1,echo=TRUE}
...
## 漢字をつかう
```{r Japanese_font2,dev="cairo_pdf",fig.width=5,fig.height=3,echo=FALSE}
print(p4)
```
...
```{r facet,dev="cairo_pdf",fig.width=5,fig.height=2.2,echo=TRUE}
...
```{r theme_bw,dev="cairo_pdf",fig.width=5,fig.height=2.5, echo=TRUE}
p4 + theme_bw(base_family = "IPAexGothic", base_size = 10)
```

## themeをかえる

```{r theme_classic,dev="cairo_pdf",fig.width=5,fig.height=2.5, echo=TRUE}
p4 + theme_classic(base_family = "IPAexGothic", base_size = 10)
```
...
p <- ggplot(data.frame(t = seq_along(x), x = x), aes(t, x))
p + geom_line()
```
...
## アラカシの年輪データ
\footnotesize データ: Itô and Sumida (2017) Ecol. Res 32:105-105.
...
```{r geom_line5, fig.width=5, fig.height=2.75, echo=TRUE}
...
```{r boxplot2, fig.width=5, fig.height=2.75, echo=TRUE}
ggplot(iris) + geom_boxplot(aes(Species, Sepal.Width)) +
  coord_flip()
```
...
p + geom_density(alpha = 0.5, show.legend = FALSE) +
  ...
  annotate("text", x = 5.8, y = 0.8, label = "versicolor") +
...
```{r stat_count, fig.width=5, fig.height=2, echo=TRUE}
set.seed(1); x <- rpois(200, 2)
ggplot(data.frame(x = x), aes(x)) + stat_count(geom = "bar")
```
...
data(Nile)
data <- data.frame(Year = start(Nile)[1]:end(Nile)[1],
                   Level = c(Nile))
library(dlm)
mod <- dlmModPoly(order = 1, dV = 100, dW = 10)
nile_filter <- dlmFilter(Nile, mod)
kf <- data.frame(Year = start(Nile)[1]:end(Nile)[1],
                 Filtered = dropFirst(nile_filter$m)) %>%
  mutate(sd = dlmSvd2var(nile_filter$U.C, nile_filter$D.C) %>%
           unlist() %>%
           dropFirst()) %>%
...
p1 <- ggplot(data) + geom_line(aes(Year, Level))
print(p1)
```

## 別のデータを重ねる

カルマンフィルタを通した値（赤線）と95%信頼区間(灰色の領域)を重ねる。

```{r geom_ribbon, fig.width=5, fig.height=2.25, echo=TRUE}
p1 + geom_line(data = kf, aes(Year, Filtered),
               colour = "red") +
  geom_ribbon(data = kf,
...
```
\footnotesize 島谷(2017) 5.3節より

## ラスター

**`geom_raster`**

```{r geom_raster, fig.width=5, fig.height=2.75, echo=TRUE}
...
```{r geom_contour, fig.width=5, fig.height=2.75, echo=TRUE}
