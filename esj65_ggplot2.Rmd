...
**``theme_grey``**関数の`base_family`引数にIPAexフォントを指定した（利用可能なフォントであればそれを指定してもよい）。
...
**`ggsave`**関数などをつかう。日本語フォント使用時にPDF出力するには`device = cairo_pdf`とする。macOSでは**`quartz.save`**関数でも。
