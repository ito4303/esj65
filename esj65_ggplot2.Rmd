...
corr_coef <- c(sprintf("%4.3f", cor(x1, y1)^2),
               sprintf("%4.3f", cor(x2, y2)^2))
text <- data.frame(X = c(0.5, 0.7), Y = c(0.1, 0.1),
                   Type = c("1", "2"),
                   labels = c(paste0("italic(r)^2==\"", corr_coef[1], "\""),
                              paste0("italic(r)^2==\"", corr_coef[2], "\"")))
ggplot(df) +
  geom_point(aes(X, Y), size = 1, alpha = 0.8) +
  geom_text(data = text, aes(X, Y, label = labels), parse = TRUE) +
  ylim(0, 1.5) +
  coord_fixed() +
  facet_wrap(~ Type)
```
...
\textcolor{red}{\bfseries \texttt{ggplot}オブジェクトに+演算子で，図を追加する。}
