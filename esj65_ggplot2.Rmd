...
\footnotesize Itô (2017) Biodiversity Data Journal 5: e11732.
...
2014年のアラカシとクロバイのDBHデータを抽出する(50m $\times$ 50mの範囲)
...
```{r coord, fig.width=5, fig.height=3, out.height="5cm", cache=TRUE}
...
```{r point_shape, fig.width=5, fig.height=3, out.height="5cm", cache=TRUE}
...
```{r point_size, fig.width=5, fig.height=3, out.height="5cm", cache=TRUE}
p2 + geom_point(aes(x = X, y = Y,
                    colour = Species, shape = Species),
                size = 4, alpha = 0.7)
```
...
```{r point_size2, fig.width=5, fig.height=3, out.height="5cm", cache=TRUE}
p3 <- p2 +
...
```{r spec_colour, fig.width=5, fig.height=3, out.height="5cm", cache=TRUE}
